...
/*
.----------------------------------------------------------------------
. Program       : EventTypes.php
.
. Function      : Event Type Maintenance
.
. Modifications :
.----------------------------------------------------------------------
PRGID     INIT      "EventTypes.php"
VERSION   INIT      "V11.05.00"
PRGNAM    INIT      "Event Type Maintanance"
.----------------------------------------------------------------------
*/
  ...
  if (isset($_POST["cmdCreate"])) {
    ...
    $SQL="INSERT INTO ICS_EventTypes 
            ( Description, Active)
...
<HEAD>
  ...
  <meta name="ServerID"        content="EventTypes.php">
  <meta name="ServerVersion"   content="V11.05.00">
  <meta name="ServerName"      content="Event Type Maintenance">
  <meta name="ServerOption"    content="N/A">
  <meta name="ServerTemplate"  content="N/A">
  <meta name="TemplateFile"    content="EventTypes.php">
  <meta name="TemplateVersion" content="V11.05.00">
  <meta name="TemplateHome"    content="php/event">
  <meta name="Title"           content="Event Type Maintenance">
<Title>EventType Maintenance</Title>
...
function init() {
 ...
 t.addColumn("Event Type ","String",1,1,"left nowrap","","","50%")
 t.addColumn("Active","String",3,3,"left nowrap","","","25%")
 OrderColumn=1; 
<?php     
   $SQL="select a.EventTypeID, a.Description, a.Active
         ,CASE a.Active WHEN 'T' THEN 'Yes' ELSE 'No' END showActive
        from ICS_EventTypes a ";
   $rs=oci_parse($GLOBALS['conn'],$SQL) or die ("Error Query [".$SQL."]");
   AddSortTable($rs);
?>
   t.addRow("<i>Insert</i>","<input type=text id=Description name=Description size=50 onblur=AddEventType();>","<input type=hidden id=Description name=Description>","","","");
 <?php
...
function rowOnclick(rowNo) {
 if (event.srcElement.className=="DeleteIcon") {
  ...
  if (confirm("Click Ok to Delete EventType")){ 
    document.form2.EventTypeID.value=t.rows[rowNo][0];
...
<form id=form2 name=form2 target=_self action=EventTypes.php method=POST>
