//jsVersion  : V11.05.00
//-----------------------------------------------------------------------------;
// Casemix Enquiry - Table Sorts and Graphs
//
// Modifications :
//
//----------------------------------------------------------------------
...
// V9.04.01  28.11.2005 Ebon Clements CAR 85942
//           Changed to allow graph to display more than 16 items
// V9.04.00  22.07.2005 Ebon Clements CAR 55997
//           CasemixEnquiry.js with arrays for TableString. CasemixEnquiryOLD.js
//           is the save copy of this before the array changes.
//----------------------------------------------------------------------
// V9.03.00  22.01.2004  Sylvek Litewka  CAR 46508
//           Created include from CasemixAnalysis.js
//
//-----------------------------------------------------------------------------
...
function SetGraphType(graphType) {
   SelectExtract.GraphType.value=graphType;
   SetGraph()
}

function SetGraph() {
   if (t.rows.length != 0 ) {
     i=SelectExtract.SetValue1.selectedIndex;
     SelectExtract.GraphValue1.value=SelectExtract.SetValue1.options[i].value;
     i=SelectExtract.SetValue2.selectedIndex;
     SelectExtract.GraphValue2.value=SelectExtract.SetValue1.options[i].value;
     TableGraph();
  }
}
...
{
  var CasemixTable = document.getElementById('CasemixTable');
...
function JavaBarGraph(GraphBase) {
   ...
   GraphString = '<applet code=javachart.applet.hyperBar.class ' +
        GraphBase + 'width=750 height=450>' +
...
function MakeDataset(GraphValue,DataSet) {
   ...
   if (t.rows.length>38) { 
     if (AscDsc=1) { lastRow=38 }
     else { startRow = t.rows.length - 37
...
function JavaPieGraph(GraphBase) {
   ...
   GraphString += '<param name=legendColor value="lightGray">' +
        ...
        '<param name=iconHeight value="0.015">' +
...
function JavaColumn2Graph(GraphBase) {
   ...
   if (document.SelectExtract.epslist.value == "0") {
     ...
     switch (SelectExtract.GraphValue1.value) {
       case "4": dataset0Name="Bed Days";break;
       case "5": dataset0Name="Income";break;
       case "6": dataset0Name="Cost";break;
     }
     switch (SelectExtract.GraphValue2.value) {
       case "4": dataset1Name="Bed Days";break;
       case "5": dataset1Name="Income";break;
       case "6": dataset1Name="Cost";break;
     }
   }
