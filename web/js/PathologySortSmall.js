...
//           PathologySortSmall.js with arrays for TableString. 
//           PathologySortSmallOLD.js is
 ...
 for(var i = 0; i < t.rows.length; i++) {
   ...
   for(var j = 0; j < t.columns.length; j++) {
     ...
     if (LinkColumn != "") {
       ...
       if (LinkHref != ""){ 
         TableString[TableString.length] = "<a href='javascript:PatientLink(\"" + 
                         t.rows[i][10] + "\");'>" ; 
         LinkFlag = 1 }
     }
...
function TableHeading(OrderColumn) {
  ...
  for(var i = 0; i < t.columns.length; i++) {
    TableString[TableString.length] = "<td class=HeadingCell align=center" +
                   " width=" + t.columns[i][7] + " ><b>" +
                   t.columns[i][0] + "</b></td>"; 
    }
  TableString[TableString.length] = "</tr>";
...
function TableEnding() 
{
  ...
  if (MoreResults) {
     TableString[TableString.length] = '<table width=100%><tr><td align=right>'
     TableString[TableString.length] = '<a href=\'javascript:ShowMore("' + MoreStartKey + '");\'>'
     TableString[TableString.length] = '...Next Page </a></td></tr></table>' 
  }
