...
function clearOptNames() {
  ...
  if (isWhitespace(document.UpdateForm.optfld17.value)) {
    document.UpdateForm.optnam17.value="";
  }
  if (!document.UpdateForm.optfld18.selectedIndex==1) {
    document.UpdateForm.optnam18.value="";
  }
  if (!document.UpdateForm.optfld19.selectedIndex==1) {
    document.UpdateForm.optnam19.value="";
  }
  if (!document.UpdateForm.optfld20.selectedIndex==1) {
    document.UpdateForm.optnam20.value="";
  }
  if (!document.UpdateForm.optfld21.selectedIndex==1) {
    document.UpdateForm.optnam21.value="";
  }
  if (!document.UpdateForm.optfld22.selectedIndex==1) {
    document.UpdateForm.optnam22.value="";
  }
  if (!document.UpdateForm.optfld23.selectedIndex==1) {
    document.UpdateForm.optnam23.value="";
  }
  if (!document.UpdateForm.optfld24.selectedIndex==1) {
    document.UpdateForm.optnam24.value="";
  }
  if (isWhitespace(document.UpdateForm.optfld25.value)) {
    document.UpdateForm.optnam25.value="";
  }
  if (!document.UpdateForm.optfld26.selectedIndex==1) {
    document.UpdateForm.optnam26.value="";
  }
}
...
function submitServiceRequest() {
  ...
  HideWaitScreen();  // perhaps some server error with AJAXOrderString()
}
function ShowSaving() {
...
function ShowPatientTests(sType) {
  ...
  xmlHttp.onreadystatechange = function() {
	...
	OS += "<li class=sectionItem  style='font-size:12px' onclick=\"AddPatientTest('"+null+"',"+
	       ...
	       displayName +
               "<span style='font-size:10px;float:right;margin-top:-5px;'>" +
	       obj[i].labname + "</span>" +
               "<br><span style='font-size:10px;line-height:10px;'>" +
               subText + "</span></li>"
      }
      ListLocation = document.getElementById("SearchResults");
...
function doctorSearch(value) {
  ...
  xmlHttp.onreadystatechange = function() {
	...
	return;
      ...
      obj = eval("("+xmlHttp.responseText.replace(/,]/,"]")+")"); //convert text to javascript object
      var OS2=" ";
      for(var i =0; i < obj.length; i++) {
  ...
  if (LookupTest(labCode,resultCode)) {
    ...
    if (!found && AdditionalFields(labCode,orderCode,orderNoteType,orderGroup)){
      ...
      if (!isWhitespace(orderSpecimen)) {
        OS += "<br><span style='font-size:10px;line-height:10px;'>"+
              orderSpecimen+"</span>"
      }
      OS += "</div>";
...
//    if (orderCodes[i].labcode==labCode&&
    if (validateLabCodeString(orderCodes[i].labcode,labCode)&&
      ...
      OS += "<li class=sectionItem style='font-size:12px' onclick=\"AddTest('"+null+"'," +
            "'"+orderCodes[i].type+"','" +
            orderCodes[i].code+"','" +
...
//         (orderCodes[i].labcode==LabCode || isWhitespace(LabCode)) &&
         (validateLabCodeString(orderCodes[i].labcode,LabCode) || isWhitespace(LabCode)) &&
...
function ShowIdValidationFrame() {
  linkurl="patweb98.pbl?reportno=1&template=216" +
          "&urnumber=" + document.PatientLinks.urnumber.value +
          "&admissno=" + document.PatientLinks.admissno;
...
function validateLabCodeString(labCodeFromJSON,labCodeString) {

  if (labCodeString.search(labCodeFromJSON)== -1) {
    return false;
  }
