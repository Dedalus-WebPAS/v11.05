...
//                      New Include
//------------------------------------------------------------
var theField, theStatus, thePosition,theCode,dSearchResult,theWidth,theHeight;
...
//
function ICD10PROSearch(proCode,proName) {
  ...
  theCode       = proCode;
  theHeight     ='250px';
  theURL        = "../cgi-bin/patweb94.pbl?reportno=04&template=004&norecord=25&keywords="+theFieldValue
...
//
function ICD10DISSearch(proCode,proName) {
  //if (document.getElementById("DivSearchResult")==null) {
  if (xGetElementById("DivSearchResult")==null) {
    document.body.insertAdjacentHTML('BeforeEnd','<div id=DivSearchResult name=DivSearchResult class=DivSearchResult></div>')
  }
  ...
  theCode       = proCode;
  theHeight     ='250px';
  theURL        = "../cgi-bin/patweb94.pbl?reportno=03&template=004&norecord=25&keywords="+theFieldValue
  keycode       = event.keyCode;
  setTimeout("AJAXSearch()", 5);
}
//
...
function DoctorSearch(docCode,docName) {
  ...
  theCode       = docCode;
  theHeight     ='250px';
  theURL        = "../cgi-bin/patweb99.pbl?reportno=02&template=011&norecord=5&doctstat=0&"+
...
function HCPSearch(docCode,docName,docStatus) {
  ...
  theHeight     ='250px';
  theCode       = docCode;
  theURL        = "../cgi-bin/patweb99.pbl?reportno=06&template=015&norecord=005&tmotfrst=20&tmotnext=20"+
                  "&hcpsstat="+docStatus+
                  "&keywords="+theFieldValue
  keycode       = event.keyCode;
  setTimeout("AJAXSearch()", 5);
...
function AJAXResults() {
  if (xmlHttp.responseText=="") {
    ...
    sHTMLOutput               = sHTMLOutput.replace(/class=HeadingCell colspan=5/g,"style='display:none';");
    sHTMLOutput               = sHTMLOutput.replace(/class=HeadingCell colspan=2/g,"style='display:none';");
    dSearchResult.style.left  = thePosition[0];
    dSearchResult.style.width = theField.offsetWidth
