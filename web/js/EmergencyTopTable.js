 ...
 for (var i = 0; i < obj.rows.length; i++) {
   ...
   for (var j = 0; j < obj.columns.length; j++) {
     if (CompatibilityMode) {
       ...
       if(obj.columns[j][9]!=undefined) { 
         if(!isWhitespace(obj.rows[i][obj.columns[j][9]])) {
           TableString[TableString.length] = " style='background-color:" +
                                      obj.rows[i][obj.columns[j][9]] + "' "
         }
       }
     } else {
       TableString[TableString.length] = "<td style='padding:0px 5px "
       if(obj.columns[j][9]!=undefined) { 
         if(!isWhitespace(obj.rows[i][obj.columns[j][9]])) {
           TableString[TableString.length] = " ;background-color:" +
                                      obj.rows[i][obj.columns[j][9]] 
         }
       }
     ...
     if (LinkColumn != "") {
       ...
       if (LinkHref != ""){ 
           if(LinkHref=="1") {
             ...
             TableString[TableString.length] = "<a onclick='javascript:SelectPatient(\"" +
                            obj.rows[i][3] + "\",\"" +
                            obj.rows[i][4] +
                            "\");'" +
           " ondblclick='javascript:SelectPatient2(\"" + 
                            obj.rows[i][3] + "\",\"" +
                            obj.rows[i][4] +
                            "\");'>"; 
                                  }
...
function TableHeading(OrderColumn,AscDsc,immediate,emergency,urgent,semi,non) {
  TableString = [];
  TableString[TableString.length] = '<div id=HeadingDivision style=\"overflow:hidden;\">'
