...
function init() {
  ...
  wurl="patweb98.pbl?reportno=1&template=115"+
  ...
  if (xmlHttp.responseText!=""&&xmlHttp.status == 200) {
    ...
    alert("OP Appointment Web Service Not available cliweb06.pbl");
  }
}
function ViewAll() {
...
function AppointmentInitTable() {
 ...
 for(var i = 0; i < t.rows.length; i++) {
   theText = "<b>Next Appointment</b><br>"+
             "<b>"+t.rows[i][1]+"</b> at "+t.rows[i][0]+"</br></br>" +
             t.rows[i][5]+"<br><br>" +
             t.rows[i][2]+" "+t.rows[i][3]+"<br><br>";

   OS += "<li class=ItemNote onclick=\"parent.AddClinicalNote(this,'"+i+"','"+theText+"');\">" +
         "<div class=ntRow><span class=ntWho>"+t.rows[i][2]+"</span>" +
         "<span class=ntType>"+t.rows[i][3]+"</span><div><br>"+
         "<div class=ntRow>"+
         "<span class=ntWhen>"+t.rows[i][5]+"</span></div>" +
         "<div class=ntRow>"+
         "<span class=ntWhen>"+t.rows[i][1]+" at "+ t.rows[i][0]+
         "</span></div></li>"
 }
 OS+='</ul>'
 ...
 if (t.rows.length == 0) {
   ListLocation.innerHTML += "<ul class=ListNote>" +
                             ...
                             "No Outpatient Appointments for this patient"+
