...
// Written   : 05.07.2011
//
// Function  : Standard Functions Used for Australian Defence Force details
//
// Mods      :
...
//        V10.02.02 31.08.2011  Jeni Tan           249641
//                  Fixed GetDefaultIns
...
function ShowBarracks() {
  if(document.UpdateForm.pmext020.value.substr(3,1) == "A") {
    Barracks.innerHTML=BarracksCodedField.innerHTML;
    document.UpdateForm.pmext023.selectedIndex=0
    document.UpdateForm.pmext023.value="";
  } else {
    Barracks.innerHTML=BarracksFreeText.innerHTML;
    document.UpdateForm.pmext078.value="";
  }
}

function GetDefaultIns(ReturnCode,ReturnIns) {
  if (isWhitespace(ReturnCode.value)) {
    ReturnIns.value="";
  } else {
    var serverURL = "../cgi-bin/patweb80.pbl?reportno=99" +
        "&valdcode=" + ReturnCode.value.substr(0,3).replace(/ /g,"+")
    var returnValue = RSExecute(serverURL);
    if (returnValue.status==0) {
      ReturnValue=returnValue.return_value.split("|");

 for (var i =0 ; i < ReturnIns.length; i++) {
  if (ReturnIns.options[i].value==ReturnValue[0]) {
       ReturnIns.selectedIndex=i 
       DisplayIns();
       } };
    }
  }
...
function ShowService() {
  ...
  if(document.UpdateForm.pmext024.value.substr(4,1) == "M") {
    ServiceCode.innerHTML=Military.innerHTML;

  for (var i =0 ; i < document.UpdateForm.pmext020.length; i++) {
   if ((document.UpdateForm.pmext020.options[i].value.substr(5,1)!="A") &&
       (document.UpdateForm.pmext020.options[i].value.substr(4,1)!="M") &&
       (document.UpdateForm.pmext020.options[i].value.substr(4,1)!=""))
      {
       document.UpdateForm.pmext020.remove(i) } };

  } else {
    ServiceCode.innerHTML=NonMilitary.innerHTML;

    for (var i =0 ; i < document.UpdateForm.pmext020.length; i++) {
     if ((document.UpdateForm.pmext020.options[i].value.substr(5,1)=="F") ||
         (document.UpdateForm.pmext020.options[i].value.substr(4,1)=="M")) {
         document.UpdateForm.pmext020.remove(i) } };
  }
...
function DisplayIns() {
  if(!isWhitespace(document.getElementById('pmext012').value)) {
    GetIns(document.getElementById('pmext012'),
     ...
     document.UpdateForm.inscname.value=""
     document.UpdateForm.insctelp.value=""
     document.UpdateForm.faxnumbr.value=""
   }
}
