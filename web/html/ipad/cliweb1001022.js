//jsVersion  : V11.05.00
//
// Source Code:  ./ipad/cliweb1001022.js
//
// Modification
//
...
function PageLoad() {
  ...
  Links = document.getElementById("jsonDocuments").innerHTML
  el = eval("("+Links+")"); //convert text to javascript object
  if (el.Link.length>0) {
    ImageLinkType='DOCS';
    ImageLinkURL=el.Link[0].url;
    el = document.getElementById("viewbutton")
    el.style.display="";
...
function ViewAttachment(theURL) {
...
function OpenDocument(linkURL) {
  if (linkURL.match(/\.pdf/i)) {
    location.href=CGIPath+"ipadpdf.php?path="+linkURL;
    return;
  }
  if (linkURL.match(/\.mpeg/i)) {
    location.href=CGIPath+"ipadmpg.php?path="+linkURL;
    return;
  }
  if (linkURL.match(/\.gif/i)||
     linkURL.match(/\.jpg/i)||
     linkURL.match(/\.png/i)) {
    location.href=CGIPath+"ipadimg.php?path="+linkURL;
    return;
  }
  alertify.alert("Document Format Not Valid on iPhone/iPad device");
}
